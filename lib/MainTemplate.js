...
function MainTemplate(outputOptions) {
	...
	this.plugin("render", function(bootstrapSource, chunk, hash, moduleTemplate, dependencyTemplates) {
		...
		chunk.modules.forEach(m => {
			if (m instanceof ExternalModule) {
		...
		if(internalModules.length === 2) {
			// Omit 'module management' boilerplate for single-module applications.
			const sourceCached = internalModules[1].source(dependencyTemplates, this.outputOptions, this.requestShortener);
			var sourceMutable = sourceCached.source();
			
			externalModules.forEach(m => {
				// Terrible hack couples us to 'require'...
				sourceMutable = sourceMutable.replace(`__webpack_require__(${m.id})`, `require('${m.request}')`);
			});
